...
def finetune_one_epoch(model, data_loader, optimizer, device, epoch, max_depth=6.7, min_depth=1e-3):
    model.train()
    epoch_loss = 0.0
    ...
    for i, (rgb, gt_depth) in pbar:
        ...
        print(f"gt_depth: {gt_depth_raw.min().item()}, {gt_depth_raw.max().item()}")
